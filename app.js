...
// 3. FITUR DAFTAR MANDIRI (LENGKAP: NAMA, WA, NIK, TGL, DLL)
...
// 4. CORE DASHBOARD & NAVIGASI (DENGAN MENU PASIEN LAMA)
function loadDashboard(role) {
    ...
    const menus = {
        admin: [
            ...
            {l:'Pasien Lama', i:'fa-history', r:renderCariPasienLama}, // FITUR BARU DITAMBAHKAN DISINI
...
// 5. RENDER LIST (FULL WIDTH + SUARA PANGGIL + CETAK RM)
...
// 6. FORM PERAWAT, DOKTER, FARMASI (FITUR LAMA DIJAGA + LABEL FARMASI)
function renderPerawat(p) {
    return `<div class="mt-8 pt-8 border-t space-y-6">
        <div class="grid grid-cols-2 gap-6">
            <input id="p-tensi-${p.id}" value="${p.medis.tensi}" placeholder="Tensi (TD)" class="p-5 bg-slate-50 border rounded-2xl font-bold">
            <input id="p-suhu-${p.id}" value="${p.medis.suhu}" placeholder="Suhu (°C)" class="p-5 bg-slate-50 border rounded-2xl font-bold">
        </div>
        <textarea id="p-riwayat-${p.id}" placeholder="Keluhan Pasien" class="w-full p-5 bg-slate-50 border rounded-2xl font-bold h-24">${p.medis.riwayat}</textarea>
...
function renderDokter(p) {
    return `<div class="mt-8 pt-8 border-t space-y-6">
        <div class="grid grid-cols-2 gap-6">
            <input id="d-icd-${p.id}" value="${p.medis.icd}" placeholder="Kode ICD-10" class="p-5 bg-slate-50 border rounded-2xl font-bold">
            <textarea id="d-resep-${p.id}" placeholder="Resep Obat (Ketik Manual)" class="p-5 bg-indigo-50 border rounded-2xl font-bold h-24">${p.medis.resep}</textarea>
        </div>
        <textarea id="d-diag-${p.id}" placeholder="Analisa Diagnosa Dokter" class="w-full p-5 bg-slate-50 border rounded-2xl font-bold h-32">${p.medis.diagnosa}</textarea>
...
function renderFarmasi(p) {
    return `<div class="mt-8 pt-8 border-t">
        <div class="p-8 bg-emerald-50 rounded-3xl mb-6 font-bold text-emerald-800 text-2xl italic">${p.medis.resep}</div>
        <div class="flex gap-4">
...
// 7. CETAKAN (REKAM MEDIS, LABEL FARMASI, INVOICE)
function printRM(id) {
    ...
    a.innerHTML = `<div class="p-10 border-4 border-black font-serif">
        <h1 class="text-center text-2xl font-black">ATRIA JAYA MEDIKA</h1><hr class="border-black my-4">
        <h3>REKAM MEDIS: ${p.nama}</h3>
        <p>NIK: ${p.nik} | Umur: ${p.umur} | Tgl: ${p.tgl_kunjungan}</p><br>
        <p><b>Keluhan:</b> ${p.medis.riwayat}</p>
        <p><b>Diagnosa:</b> ${p.medis.diagnosa} (ICD: ${p.medis.icd})</p>
        <p><b>Terapi:</b> ${p.medis.resep}</p>
    </div>`;
    window.print();
}
...
function printEtiket(id) {
    ...
    a.innerHTML = `<div class="p-6 border-2 border-blue-600 rounded-xl w-[300px] text-xs font-bold text-center">
        ...
        <p class="text-left">Nama: ${p.nama}</p><p class="text-left">Tgl: ${p.tgl_kunjungan}</p>
        <div class="bg-blue-50 p-4 my-2 text-lg font-black border border-blue-200">${p.medis.resep}</div>
        <p class="italic">Semoga Cepat Sembuh</p>
    </div>`;
    window.print();
}

// 8. MONITOR TV (FITUR PATEN)
function toggleTV() { 
...
// 9. FITUR RIWAYAT KERJA PER PETUGAS (MENU BARU)
function renderRiwayatPetugas(target) {
    ...
    return `<div class="bg-white p-10 rounded-[3rem] shadow-sm">
        <h3 class="font-black mb-6 uppercase">Riwayat Kerja Saya</h3>
        ...
        </div>`).join('') || 'Belum ada data'}
    </div>`;
}

// 10. UTILITAS LAINNYA
function renderAdmisi() { 
    return `<div class="bg-white p-10 rounded-[3rem] shadow-sm"><h3 class="text-2xl font-black mb-8 text-blue-600 uppercase">Pendaftaran Admisi</h3>
    <div class="grid grid-cols-2 gap-6">
        <input id="a-nama" placeholder="Nama Pasien" class="p-4 bg-slate-50 border rounded-xl font-bold">
    ...
    <button onclick="simpanAdmisi()" class="mt-6 bg-blue-600 text-white px-8 py-4 rounded-xl font-black uppercase text-xs">Daftarkan</button></div>`; 
}
...
function savePerawat(id) { const i=dbPasien.findIndex(x=>x.id==id); dbPasien[i].medis.tensi=document.getElementById(`p-tensi-${id}`).value; dbPasien[i].medis.suhu=document.getElementById(`p-suhu-${id}`).value; dbPasien[i].medis.riwayat=document.getElementById(`p-riwayat-${id}`).value; dbPasien[i].status='Antre Dokter'; save(); loadDashboard('perawat'); }
function saveDokter(id) { const i=dbPasien.findIndex(x=>x.id==id); dbPasien[i].medis.icd=document.getElementById(`d-icd-${id}`).value; dbPasien[i].medis.resep=document.getElementById(`d-resep-${id}`).value; dbPasien[i].medis.diagnosa=document.getElementById(`d-diag-${id}`).value; dbPasien[i].status='Antre Farmasi'; save(); loadDashboard('dokter'); }
function printInvoice(id) { const p=dbPasien.find(x=>x.id==id); const a=document.getElementById('print-area'); a.innerHTML=`<div class="p-6 border font-mono"><h3>STRUK ATRIA JAYA</h3><p>${p.nama}</p><h3>TOTAL: Rp ${p.billing.toLocaleString()}</h3></div>`; window.print(); updateStat(id, 'Selesai'); }
...
function revisi(id, s) { const i=dbPasien.findIndex(x=>x.id==id); dbPasien[i].status=s; save(); loadDashboard(JSON.parse(sessionStorage.getItem('artha_session')).role); }
...
function renderRiwayatUmum() { return `<div class="bg-white p-10 rounded-3xl overflow-hidden"><table class="w-full text-left">${dbPasien.map(p=>`<tr class="border-b p-4"><td>#${p.no}</td><td>${p.nama}</td><td>${p.status}</td></tr>`).join('')}</table></div>`; }
function renderOwner() { const tot=dbPasien.filter(p=>p.status==='Selesai').reduce((a,b)=>a+b.billing,0); return `<div class="bg-blue-600 p-16 rounded-[4rem] text-white"><h2>Omset Klinik: Rp ${tot.toLocaleString()}</h2></div>`; }
